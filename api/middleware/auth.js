...
exports.authResume = asyncHandler(async (req, res, next) => {
    ...
    if (tokenUser && tokenUser !== 'none') {
        ...
        const authorization = await Authorization.findOne({ user_id: user.id, resume_id: resume.id })
        console.log('authorization: ', authorization)
        if (authorization) {
            console.log('OK continue, authenticated user a has valid authorization')
            req.userResume = resume
            return next()
        }
    }
...
exports.authorize = (...roles) => {
    ...
    return (req, res, next) => {
        if (!roles.includes(req.user.role)) {
            return next(new ErrorResponse(`User role ${req.user.role} is not authorized to access this route 2`, 403))
        }
        next()
    }
}
